...
CENTER_X = 256
CENTER_Y = 256
FLAG_SPAWN_POS = (CENTER_X, CENTER_Y)
...
def apply_script(protocol, connection, config):
    ...
    class OneCTFProtocol(protocol):
        ...
        def __init__(self, *arg, **kw):
            ...
            self.reset_flags()
            self.reverse_one_ctf_messages()

        def reverse_one_ctf_messages(self):
            self.tips.append(REVERSE_CTF_MESSAGE)
            self.help.append(REVERSE_CTF_MESSAGE)
        ...
        def onectf_reset_flag(self, flag):
            z = self.map.get_z(*FLAG_SPAWN_POS)
