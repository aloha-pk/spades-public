    ...
    try:
        ...
        if command in ('add', 'set'):
            ...
            if action in ('height', 'raise', 'lower', 'elevator'):
                ...
                # disallow teleports in f6
                if x >= 320.0 and x <= 383.0 and y >= 320.0 and y <= 383.0:
                    raise ValueError('you can not teleport to this location :(')
                z = max(0.5, z)
...
@admin
def rplatforms(connection, confirmation = None):
    pCoords = to_coordinates(*(connection.get_location()[:2]))
    if confirmation == None:
        return "This will remove all buttons and platforms for this sector. Type /rplatforms %d to continue." % connection.confirmation
    if int(confirmation) != connection.confirmation:
        return "Invalid confirmation code. Type /rplatforms %d if you want to clear this sector." % connection.confirmation
    removal = []
    for button in connection.protocol.buttons:
        butt = connection.protocol.buttons[button]
        if to_coordinates(butt.x, butt.y) == pCoords:
            butt.destroy()
            removal.append(button)
    for button in removal:
        del connection.protocol.buttons[button]
    removal = []
    for platform in connection.protocol.platforms:
        plat = connection.protocol.platforms[platform]
        if to_coordinates(plat.x1, plat.y1) == pCoords:
            plat.destroy()
            removal.append(platform)
    for platform in removal:
        del connection.protocol.platforms[platform]
    return "Sector cleared successfully."

for func in (platform_command, button_command, action_command,
    trigger_command, save, reach, rplatforms):
    add(func)
...
class NewPlatformState(State):
    ...
    def on_exit(self, protocol, player):
        if player.claim.count(to_coordinates(*(player.get_location()[:2]))) == 0: #if their location is not in claims
            return "You do not have access to this sector."
        if not self.blocks:
