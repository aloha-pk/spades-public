...
def kick_afk(connection, minutes, amount = None):
    ...
    for conn in protocol.connections.values():
        if not conn.admin and not (conn.user_types is not None and (conn.user_types.moderator or conn.user_types.guard or conn.user_types.trusted)) and conn.last_activity < lower_bound:
            to_kick.append(conn)
    if not to_kick:
