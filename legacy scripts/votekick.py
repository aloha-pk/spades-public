...
'''
Modified for www.aloha.pk

Changelog:
6.15.14 by ei8htx
Added functionality so votekicks are counted by IP (to prevent multi-client gang bangs)
...
class Votekick(object):
    ...
    def start(cls, instigator, victim, reason = None):
        ...
        elif protocol.get_required_votes() <= 2:
    ...
    def __init__(self, instigator, victim, reason):
        ...
        self.victim = victim
        self.reason = reason
        self.votes = {instigator.address[0] : True}
        self.ended = False
    ...
    def vote(self, player):
        if self.victim is player:
            return
        elif player.address[0] in self.votes:
            return
        #print("self.votes: ", self.votes)	#testing
        if self.public_votes:
            self.protocol.send_chat(S_YES.format(player = player.name))
        self.votes[player.address[0]] = True
        if self.votes_remaining <= 0:
            ...
            if self.ban_duration > 0.0:
                victim.ban(self.reason + " (Votekick)", self.ban_duration)
            else:
        ...
        def on_disconnect(self):
            ...
            if votekick:
                if votekick.victim is self:
                    ...
                    self.ban(reason + " (Votekick)", Votekick.ban_duration)
