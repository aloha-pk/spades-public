    ...
    class Aimbot2Connection(connection):
        ...
        def warn_admin(self, prefix = 'Possible aimbot detected.'):
            ...
            for player in self.protocol.players.values():
                if (player.admin or (player.user_types is not None and (player.user_types.guard or player.user_types.moderator))):
        ...
        def on_hit(self, hit_amount, hit_player, type, grenade):
            if self.team is not hit_player.team:
                if type == WEAPON_KILL or type == HEADSHOT_KILL:
                    current_time = reactor.seconds()
                    shotgun_use = False
                    if current_time - self.shot_time > (0.2 * hit_player.weapon_object.delay):
                        shotgun_use = True
                        self.multiple_bullets_count = 0
